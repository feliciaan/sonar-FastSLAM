...
def calculate_pose(old_pose, motion, timedelta):
    ...
    new_x_with_noise = random.gauss(old_pose.x + dx, STRAIGHT_GAUSS) if dx != 0 else old_pose.x
    new_y_with_noise = random.gauss(old_pose.y + dy, STRAIGHT_GAUSS) if dy != 0 else old_pose.y
    new_theta_with_noise = random.gauss(old_pose.theta + dtheta, ANGLE_GAUSS) if dtheta != 0 else old_pose.theta
